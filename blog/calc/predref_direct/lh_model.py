...
def dir_problem(Eref, k_ref, Emet, k_met, degree, B):
    ...
    for exp_number in range(1, 5):
        if exp_number == 1:
            ...
            xexp = np.array(
                [[86.425, 80.285, 72.215, 64.544, 59.181, 52.588, 41.744, 30.487, 28.250, 20.715, 11.281, 5.098, 1.101],
                 # C3H8
                 ...
                 [4.008, 6.933, 10.831, 18.492, 20.167, 25.260, 35.053, 53.883, 45.900, 53.517, 62.496, 66.652, 70.908]
                 # CH4
                 ]) / 100
        ...
        if exp_number == 2:
            ...
            xexp = np.array(
                [[89.685, 82.697, 74.778, 67.309, 62.231, 55.728, 47.813, 40.215, 32.368, 26.326, 13.824, 10.190, 7.788,
                  5.184, 2.996, 1.526],  # C3H8
                 [2.923, 4.285, 5.800, 7.200, 8.330, 9.293, 10.560, 11.709, 13.031, 13.528, 15.973, 16.648, 17.168, 17.961,
                  18.051, 18.389],  # CO2
                 [5.363, 7.690, 10.426, 12.262, 14.010, 13.859, 13.859, 12.910, 13.120, 12.047, 12.847, 13.504, 14.403,
                  15.520, 16.802, 16.783],  # H2
                 [1.982, 5.220, 8.835, 13.027, 15.230, 20.860, 27.479, 34.905, 41.230, 47.837, 57.214, 59.547, 60.554,
                  61.279, 62.117, 63.302]  # CH4
                 ]) / 100
            GHSV = 12000  # wet
            L = 3.3  # высота слоя, см
            P0 = 1.05
        ...
        filename = 'lang_model' + str(exp_number) + '.png'
